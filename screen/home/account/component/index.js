...
export default function Index(props) {
    ...
    const onRouteHandler = (index) => {
        ...
        if (index === 0) {
            ...
            return Alert.alert('you want to logout?', '', [{ text: 'Cancel' }, { text: 'YES', onPress: () => signOut() }]);
        }
    }
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.header}>
                ...
                <MaterialCommunityIcons name='account-circle' size={50} color={'#0090D6'} />
            </View>
            <View style={styles.row}>
                {row.map((e, index) =>
                    <TouchableOpacity style={styles.list} activeOpacity={0.6} key={index} onPress={() => onRouteHandler(index)}>
                        <View style={styles.leftSideList}>
                            <MaterialCommunityIcons name={icon[index]} size={20} color={'#e91e63'} style={styles.icon} />
